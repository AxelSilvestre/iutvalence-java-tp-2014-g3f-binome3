...
public class Plateau
{
	...
	{
		return this.cases[unePosition.obtenirCoordonneesEnX()][unePosition
				.obtenirCoordonneesEnY()];
	}
		...
		for (int numeroDeLigne = 0; numeroDeLigne < NOMBRE_DE_LIGNES; numeroDeLigne++)
		{
			...
			for (int numeroDeColonne = 0; numeroDeColonne < NOMBRE_DE_COLONNES; numeroDeColonne++)
			{
				if (this.cases[numeroDeColonne][numeroDeLigne].obtenirPion() != null)
					plateauAsciiArt += this.cases[numeroDeColonne][numeroDeLigne]
							.obtenirPion().obtenirCouleur().toString();
				else
					plateauAsciiArt += "+ ";
			}
			plateauAsciiArt += "\n";
